<template>
  <div class="h-full bg-surface flex flex-col relative">
    <!-- Header -->
    ...
    <div class="flex-1 flex flex-col items-center overflow-y-auto px-4 pb-32">
      ...
      <div class="w-72 flex justify-center">
        <svg width="240" height="80" viewBox="0 0 240 80">
          ...
          <!-- Cockpit windows -->
          <rect x="100" y="35" width="16" height="10" rx="3" fill="rgba(255,255,255,0.08)"
            stroke="rgba(255,255,255,0.15)" stroke-width="0.5" />
          <rect x="124" y="35" width="16" height="10" rx="3" fill="rgba(255,255,255,0.08)"
            stroke="rgba(255,255,255,0.15)" stroke-width="0.5" />
        </svg>
      </div>
      ...
      <div class="w-72 border-x border-white/10 bg-white/[0.02]">
        <div v-for="row in 20" :key="row" class="flex items-center justify-center gap-1 py-[3px] px-3">
          <!-- Left seats (A, B, C) -->
          <button v-for="col in leftCols" :key="`${row}${col}`"
            class="w-9 h-8 rounded-md text-[10px] font-mono font-bold transition-all" :class="getSeatClass(row, col)"
            :disabled="isTaken(row, col)" @click="selectSeat(row, col)">
            {{ isTaken(row, col) ? '' : col }}
          </button>
          ...
          <!-- Right seats (D, E, F) -->
          <button v-for="col in rightCols" :key="`${row}${col}`"
            class="w-9 h-8 rounded-md text-[10px] font-mono font-bold transition-all" :class="getSeatClass(row, col)"
            :disabled="isTaken(row, col)" @click="selectSeat(row, col)">
            {{ isTaken(row, col) ? '' : col }}
          </button>
    ...
    <div
      class="absolute bottom-0 inset-x-0 z-50 bg-gradient-to-t from-surface via-surface/95 to-transparent pt-8 pb-8 px-6">
      <!-- Legend -->
      ...
      <button :disabled="!selectedSeatLabel"
