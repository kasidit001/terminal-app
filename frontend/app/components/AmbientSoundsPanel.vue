    ...
    <div class="space-y-6">
      ...
      <div class="flex items-center justify-between">
        <div class="flex items-center gap-3">
          ...
          <button class="px-3 py-1 rounded-lg text-xs font-bold transition-all" :class="sounds.isEnabled.value
            ? 'bg-flight-400 text-black'
            : 'bg-white/10 text-gray-400'" @click="sounds.toggleMaster()">
      ...
      <div>
        ...
        <div class="grid grid-cols-2 gap-3">
          <button v-for="track in whiteNoiseTracks" :key="track.id" class="glass-card p-4 text-left transition-all"
            ...
            <div class="flex items-center justify-between mb-2">
              ...
              <span class="text-[10px] font-bold px-2 py-0.5 rounded"
                :class="track.enabled ? 'bg-flight-400 text-black' : 'bg-white/10 text-gray-500'">{{ track.enabled ?
                'ON' : 'OFF' }}</span>
            </div>
      ...
      <div>
        ...
        <div class="grid grid-cols-2 gap-3">
          <button v-for="track in natureTracks" :key="track.id" class="glass-card p-4 text-left transition-all"
            :class="track.enabled ? 'border-flight-400/40 bg-flight-400/10' : 'hover:bg-white/10'"
            @click="sounds.toggleTrack(track.id)">
            <div class="flex items-center justify-between mb-2">
              <span class="text-xl">{{ soundIcons[track.id] || '🔊' }}</span>
